/**
 * Multi-block SEARCH/REPLACE diff strategy for the apply_diff tool.
 *
 * Ported from the reference repo's core/diff/strategies/multi-search-replace.ts.
 * Adaptations for this (zero-runtime-dependency) project:
 *   - `fastest-levenshtein`'s `distance`  -> local ./levenshtein#levenshteinDistance
 *   - extract-text line-number helpers    -> local ./line-numbers
 *   - text-normalization#normalizeString   -> local ./text-normalization
 *   - the DiffStrategy class + getProgressStatus (which depended on Roo-only
 *     ToolUse / ToolProgressStatus types) are dropped in favour of a single
 *     standalone `applyDiff` function returning a plain DiffResult.
 *
 * The matching algorithm (marker-sequencing validation, middle-out fuzzy
 * search, aggressive line-number stripping, indentation preservation) is
 * preserved verbatim.
 */

import { levenshteinDistance } from "./levenshtein"
import { addLineNumbers, everyLineHasLineNumbers, stripLineNumbers } from "./line-numbers"
import { normalizeString } from "./text-normalization"

/** Result of attempting to apply a diff to a file. */
export interface DiffResult {
	success: boolean
	/** New file content on success. */
	content?: string
	/** Error message on failure. */
	error?: string
	/** Per-block failures when some blocks applied and others did not. */
...
function getSimilarity(original: string, search: string): number {
	...
	const dist = levenshteinDistance(normalizedOriginal, normalizedSearch)
...
function validateMarkerSequencing(diffContent: string): { success: boolean; error?: string } {
	const State = { START: 0, AFTER_SEARCH: 1, AFTER_SEPARATOR: 2 } as const
	type StateValue = (typeof State)[keyof typeof State]
	const state = { current: State.START as StateValue, line: 0 }
	...
	const expectedFor = (s: StateValue): string =>
		s === State.AFTER_SEARCH ? "=======" : ">>>>>>> REPLACE"
	...
	return state.current === State.START
		...
		: {
				success: false,
				error: `ERROR: Unexpected end of sequence: Expected '${expectedFor(state.current)}' was not found.`,
			}
}

/**
 * Apply one or more SEARCH/REPLACE blocks to `originalContent`.
 *
 * @param originalContent The current file content.
 * @param diffContent     The diff text (one or more <<<<<<< SEARCH … >>>>>>> REPLACE blocks).
 * @param fuzzyThreshold  Minimum similarity (0..1) for a fuzzy match. Default 1.0 (exact).
 * @param bufferLines     Context window (lines) used when searching around a given start_line.
 * @returns A DiffResult. On partial success, `content` holds the new text and `failParts`
 *          describes the blocks that failed; on total failure, `failParts` lists every block.
 */
export async function applyDiff(
	originalContent: string,
	diffContent: string,
	fuzzyThreshold: number = 1.0,
